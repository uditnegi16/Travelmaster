...
class HotelIntelligenceEngine:
    ...
    def _analyze_market(self, hotels: List[HotelOption]) -> Dict[str, Any]:
        ...
        star_distribution = {}
        for s in range(1, 6):
            star_distribution[f"{s}_star"] = sum(1 for hotel in hotels if hotel.stars == s)
    ...
    def _analyze_price(
        ...
        # Is it a deal?
        is_deal = savings > avg_price * 0.2 and hotel.stars >= 3
        
        # Value score (0-10)
        # Higher stars and amenities with lower price = better value
        star_value = hotel.stars / 5.0 * 3  # 0-3 points
    ...
    def _compute_quality_score(
        ...
        # Rating score (0-10)
        rating_score = (hotel.stars / 5.0) * 10
        ...
        # Explanation
        if overall >= 8:
            explanation = f"Excellent {hotel.stars}-star hotel with great amenities and value"
        elif overall >= 6:
            explanation = f"Good {hotel.stars}-star hotel with solid amenities"
        else:
            explanation = f"Basic {hotel.stars}-star accommodation"
    ...
    def _compute_match_score(
        ...
        if preferences:
            ...
            if quality_pref == "high" and hotel.stars >= 4:
                base_score += 5
    ...
    def _generate_tags(
        ...
        # Quality-based tags
        max_stars = max(h.stars for h in all_hotels)
        if hotel.stars == max_stars and hotel.stars >= 4:
    ...
    def _determine_best_for(self, hotel: HotelOption, amenities: AmenitiesAnalysis) -> List[str]:
        ...
        # Luxury seekers
        if hotel.stars >= 4 and amenities.unique_features:
    ...
    def _generate_summary(self, hotel: HotelOption, tags: List[str]) -> str:
        ...
        summary = f"{hotel.name} • {hotel.stars}★ • ₹{hotel.price_per_night:,}/night"
    ...
    def _generate_insights(
        ...
        if price_intel.is_deal:
            insights.append(HotelInsight(
                ...
                message=f"Excellent value - {hotel.stars}★ hotel at {price_intel.price_category} prices",
                priority=4
            ))
    ...
    def _generate_recommendations(
        ...
        if hotel.stars >= 4:
            recommendations.append(HotelRecommendation(
                reason=f"{hotel.stars}-star rating ensures quality service and comfort",
                category=REC_CATEGORY_RATING,
                sentiment=SENTIMENT_POSITIVE
            ))
        elif hotel.stars <= 2:
            recommendations.append(HotelRecommendation(
                reason=f"{hotel.stars}-star rating - basic accommodation with limited services",
                category=REC_CATEGORY_RATING,
    ...
    def _determine_best_choice(self, enriched_hotels: List[EnrichedHotel]) -> str:
        ...
        return (
            f"{hotel.name} (Rank #1) - {hotel.stars}★ at ₹{hotel.price_per_night:,}/night "
