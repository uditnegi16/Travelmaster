...
def search_places_google_api(
    ...
    Example:
        ...
        "Eiffel Tower"
    """
    # FIX: graceful fallback instead of crashing — lets dataset adapter take over
    if not GOOGLE_MAPS_API_KEY:
        logger.warning("GOOGLE_MAPS_API_KEY not configured — places will use dataset fallback")
        return []
    ...
    try:
        ...
        # FIX: expanded field mask — was missing formattedAddress (used by normalizer)
        # and editorialSummary (used for description). Also added maxResultCount.
        headers = {
            ...
            "X-Goog-FieldMask": (
                ...
                "places.photos,places.userRatingCount"
            ),
        }
        ...
        body = {
            ...
            "maxResultCount": min(limit, 20),  # FIX: was missing — API returned default 1 result
        }
