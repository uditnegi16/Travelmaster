...
class TravelPlannerOrchestrator:
    ...
    def plan_trip(self, request: TripRequest, planner_output=None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 2: Execute services (flights, hotels, places, weather)
            debug.setdefault("enriched", {})  # ✅ do NOT reset it

            flights_raw, flights_time = self._run_flights(plan, debug)
            flights = flights_raw or []
            debug["timings"]["flights"] = flights_time
            debug["counts"]["flights"] = len(flights)

            hotels_raw, hotels_time = self._run_hotels(plan, debug)
            hotels = hotels_raw or []
            debug["timings"]["hotels"] = hotels_time
            debug["counts"]["hotels"] = len(hotels)

            places_raw, places_time = self._run_places(plan, debug)
            places = places_raw or []
            debug["timings"]["places"] = places_time
            debug["counts"]["places"] = len(places)

            weather_raw, weather_time = self._run_weather(plan, debug)
            weather = weather_raw or []
            debug["timings"]["weather"] = weather_time
            debug["counts"]["weather"] = len(weather)
    ...
    def _run_planner(self,query: str,debug: Dict[str, Any]) -> tuple[Any, float]:
        """
    ...
    def _run_flights(
        ...
        plan: Any,
        debug: Dict[str, Any]
    ) -> tuple[Optional[List[FlightOption]], float]:

        logger.info("Step 2a: Running flight search")
        start = time.time()

        try:
            flight_tool = next((tc for tc in plan.tool_calls if tc.tool_name == "search_flights"), None)

            if not flight_tool:
                debug.setdefault("warnings", []).append("No flight search in plan")
                logger.warning("No flight search requested")
                return None, time.time() - start

            # Raw list (THIS is what API needs)
            flights_raw = call_local_tool("search_flights", flight_tool.arguments)

            # Enrich ONLY for debug/composer (never replace the list)
            if flights_raw:
                try:
                    flights_enriched = enrich_flights(flights_raw)
                    debug.setdefault("enriched", {})["flights"] = flights_enriched
                    logger.info(f"Enriched {len(flights_raw)} flights")
                except Exception as e:
                    debug.setdefault("warnings", []).append(f"Flight enrichment failed: {e}")
                    logger.warning(f"Flight enrichment failed, continuing with raw flights: {e}")

            elapsed = time.time() - start
            logger.info(f"Flight search complete: {len(flights_raw) if flights_raw else 0} results ({elapsed:.2f}s)")
            return flights_raw, elapsed

        except Exception as e:
            elapsed = time.time() - start
            logger.error(f"Flight search failed: {e}")
            debug.setdefault("warnings", []).append(f"Flight search failed: {e}")
            return None, elapsed
    ...
    def _run_hotels(
        ...
        plan: Any,
        debug: Dict[str, Any]
    ) -> tuple[Optional[List[HotelOption]], float]:
        """
        ...
        try:
            ...
            # Apply enrichment
            # Apply enrichment, but keep it ONLY for debug/composer (do not replace the hotel list)
            hotels_raw = call_local_tool("search_hotels", hotel_tool.arguments)

            # Enrich ONLY for debug/composer (never replace the HotelOption list)
            if hotels_raw and len(hotels_raw) > 0:
                try:
                    hotels_enriched = enrich_hotels(hotels_raw)
                    debug.setdefault("enriched", {})["hotels"] = hotels_enriched
                    logger.info(f"Enriched {len(hotels_raw)} hotels")
                except Exception as e:
                    debug.setdefault("warnings", []).append(f"Hotel enrichment failed: {e}")
                    logger.warning(f"Hotel enrichment failed, continuing with raw hotels: {e}")

            elapsed = time.time() - start
            logger.info(f"Hotel search complete: {len(hotels_raw) if hotels_raw else 0} results ({elapsed:.2f}s)")
            return hotels_raw, elapsed
    ...
    def _run_places(
        ...
        plan: Any,
        debug: Dict[str, Any]
    ) -> tuple[Optional[List[PlaceOption]], float]:

        logger.info("Step 2c: Running places search")
        start = time.time()

        try:
            places_tool = next((tc for tc in plan.tool_calls if tc.tool_name == "search_places"), None)

            if not places_tool:
                debug.setdefault("warnings", []).append("No places search in plan")
            ...
            if places_raw:
                try:
                    ...
                    debug.setdefault("enriched", {})["places"] = places_enriched
                    logger.info(f"Enriched {len(places_raw)} places")
                except Exception as e:
                    debug.setdefault("warnings", []).append(f"Places enrichment failed: {e}")
                    logger.warning(f"Places enrichment failed, continuing with raw places: {e}")

            elapsed = time.time() - start
            logger.info(f"Places search complete: {len(places_raw) if places_raw else 0} results ({elapsed:.2f}s)")
            return places_raw, elapsed

        except Exception as e:
            elapsed = time.time() - start
            logger.error(f"Places search failed: {e}")
            debug.setdefault("warnings", []).append(f"Places search failed: {e}")
            return None, elapsed
    ...
    def _run_weather(
        ...
        plan: Any,
        debug: Dict[str, Any]
    ) -> tuple[Optional[List[WeatherSummary]], float]:

        logger.info("Step 2d: Running weather forecast")
        start = time.time()

        try:
            weather_tool = next((tc for tc in plan.tool_calls if tc.tool_name == "get_weather_forecast"), None)

            if not weather_tool:
                debug.setdefault("warnings", []).append("No weather forecast in plan")
            ...
            if weather_raw:
                try:
                    ...
                    debug.setdefault("enriched", {})["weather"] = weather_enriched
                    logger.info(f"Enriched {len(weather_raw)} weather forecasts")
                except Exception as e:
                    debug.setdefault("warnings", []).append(f"Weather enrichment failed: {e}")
                    logger.warning(f"Weather enrichment failed, continuing with raw weather: {e}")

            elapsed = time.time() - start
            logger.info(f"Weather forecast complete: {len(weather_raw) if weather_raw else 0} results ({elapsed:.2f}s)")
            return weather_raw, elapsed

        except Exception as e:
            elapsed = time.time() - start
            logger.error(f"Weather forecast failed: {e}")
            debug.setdefault("warnings", []).append(f"Weather forecast failed: {e}")
            return None, elapsed
    ...
    def _build_itinerary(
        ...
        try:
            ...
            # Try hotel check-in/out first (best for trip duration)
            if hotel and getattr(hotel, "check_in", None) and getattr(hotel, "check_out", None):
                try:
                    raw_ci = hotel.check_in
                    raw_co = hotel.check_out

                    check_in = datetime.strptime(raw_ci, "%Y-%m-%d") if isinstance(raw_ci, str) else raw_ci
                    check_out = datetime.strptime(raw_co, "%Y-%m-%d") if isinstance(raw_co, str) else raw_co
            ...
            if not dates and flight:
                ...
                if not dates:
                    dep = getattr(flight, "departure_date", None)
                    ret = getattr(flight, "return_date", None)

                    if dep and ret:
                        try:
                            start_date = datetime.strptime(dep, "%Y-%m-%d") if isinstance(dep, str) else dep
                            end_date = datetime.strptime(ret, "%Y-%m-%d") if isinstance(ret, str) else ret
            ...
            if not dates and plan and hasattr(plan, "tool_calls"):
                # First try hotel check_in / check_out from the plan
                for tool_call in plan.tool_calls:
                    if tool_call.tool_name == "search_hotels":
                        args = tool_call.arguments
                        ci = args.get("check_in")
                        co = args.get("check_out")
                        if ci and co:
                            try:
                                start_dt = datetime.strptime(ci, "%Y-%m-%d")
                                end_dt   = datetime.strptime(co, "%Y-%m-%d")
                                current = start_dt
                                while current <= end_dt:
                                    dates.append(current.strftime("%Y-%m-%d"))
                                    current += timedelta(days=1)
                                logger.info(f"Derived {len(dates)} dates from planner hotel check_in/out")
                                break
                            except Exception as e:
                                logger.warning(f"Planner hotel date extraction failed: {e}")

                # Then try flight 'date' + compute_budget 'nights'
                if not dates:
                    flight_date = None
                    nights = 3  # sensible default
                    for tool_call in plan.tool_calls:
                        if tool_call.tool_name == "search_flights":
                            flight_date = tool_call.arguments.get("date") or tool_call.arguments.get("start_date")
                        if tool_call.tool_name == "compute_budget":
                            n = tool_call.arguments.get("nights")
                            if n and str(n).isdigit():
                                nights = int(n)
                    if flight_date:
                        try:
                            start_dt = datetime.strptime(flight_date, "%Y-%m-%d")
                            for i in range(max(1, nights + 1)):
                                dates.append((start_dt + timedelta(days=i)).strftime("%Y-%m-%d"))
                            logger.info(f"Derived {len(dates)} dates from flight date + {nights} nights")
                        except Exception as e:
                            logger.warning(f"Planner flight date extraction failed: {e}")

                # Original fallback: start_date/end_date keys
                if not dates:
                    for tool_call in plan.tool_calls:
                        if tool_call.tool_name == "search_flights":
                            args = tool_call.arguments
                            if "start_date" in args and "end_date" in args:
                                try:
                                    ...
                                    while current <= end_date:
                                        dates.append(current.strftime("%Y-%m-%d"))
                                        current += timedelta(days=1)
                                    logger.info(f"Derived {len(dates)} dates from planner start_date/end_date")
                                    break
                                except Exception as e:
            ...
            if hasattr(enriched, "day_schedules"):
                for day_schedule in enriched.day_schedules:
                    # Normalize activities to the PlaceOption type expected by DayPlan
                    activities_fixed = []
                    for a in day_schedule.activities:
                        if hasattr(a, "model_dump"):
                            activities_fixed.append(PlaceOption.model_validate(a.model_dump()))
                        else:
                            activities_fixed.append(PlaceOption.model_validate(a))
            ...
            budget_summary = budget_summary if budget_summary else BudgetSummary(
                ...
                hotel_cost=0,
                activities_cost=0,
                enrichment=debug.get("enriched", {}).get("budget"),
            )

            trip_response = TripResponse(
                trip_plan=trip_plan,
                budget_summary=BudgetSummary.model_validate(budget_summary.model_dump()),
    ...
    def _run_composer(self,trip_response: TripResponse,debug: Dict[str, Any]) -> tuple[str, float]:
        """
...
def generate_trip_plan(
    user_query: Optional[str] = None,
    trip_request: Optional[Union[TripRequest, Dict[str, Any]]] = None,
) -> dict:
    if not user_query and not trip_request:
        raise ValueError("Either user_query or trip_request must be provided")

    # ✅ normalize dict -> TripRequest (fixes dict attribute error)
    if isinstance(trip_request, dict):
        trip_request = TripRequest(**trip_request)

    if trip_request:
        result = _orchestrator.plan_trip(trip_request)   # expects TripRequest object
    else:
        result = _orchestrator.plan_trip_from_text(user_query or"")
