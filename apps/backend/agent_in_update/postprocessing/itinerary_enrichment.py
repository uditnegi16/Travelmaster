...
def categorize_weather(ws: Optional[WeatherSummary]) -> str:
    if not ws:
        return "UNKNOWN"
    # example logic:
    if getattr(ws, "rain_chance", 0) >= 0.5:
        return "RAINY"
    return (getattr(ws, "condition", "") or "CLEAR").upper()
...
#     for ws in (weather_summaries or []):
#         # ws can be dict or WeatherSummary model
#         if isinstance(ws, dict):
#             d = ws
#         elif hasattr(ws, "model_dump"):
#             d = ws.model_dump()
...
#             if dt:
#                 weather_by_date[str(dt)] = d
#             ws_dict = weather_by_date.get(str(date))
#             ws_model = None
#             if ws_dict is not None:
#                 # This reconstructs WeatherSummary using the correct class import
#                 ws_model = WeatherSummary.model_validate(ws_dict)  # pydantic v2
#                 # for pydantic v1 use: ws_model = WeatherSummary.parse_obj(ws_dict)
...
def analyze_day_characteristics(
    dates: List[str],
    flight: Optional[FlightOption],
    weather_summaries: List[Union[WeatherSummary, Dict[str, Any]]],
) -> List[DayCharacteristics]:
    characteristics: List[DayCharacteristics] = []

    # 1) normalize -> Dict[date, WeatherSummary]
    weather_by_date: Dict[str, WeatherSummary] = {}
    for ws in (weather_summaries or []):
        if isinstance(ws, WeatherSummary):
            model = ws
        elif isinstance(ws, dict):
            model = WeatherSummary.model_validate(ws)   # pydantic v2
        else:
            continue

        if getattr(model, "date", None):
            weather_by_date[str(model.date)] = model

    # 2) build day characteristics
    for idx, dt in enumerate(dates):
        day_num = idx + 1
        ws_model: Optional[WeatherSummary] = weather_by_date.get(str(dt))

        # categorize_weather should accept WeatherSummary | None
        weather_cat = categorize_weather(ws_model)
        ...
        if is_first and flight:
            ...
            available_hours = (
                get_available_hours_before_departure(departure_time)
                if departure_time else 4.0
            )
            max_activities = MAX_ACTIVITIES_DEPARTURE
            intensity = INTENSITY_LIGHT
            ...
            max_activities = MAX_ACTIVITIES_FULL_DAY
            intensity = (
                INTENSITY_MODERATE if idx % 3 == 0
                else INTENSITY_HEAVY if idx % 3 == 1
                else INTENSITY_LIGHT
            )
        ...
        characteristics.append(
            DayCharacteristics(
                ...
                day_type=day_type,
                weather_condition=weather_cat,
                weather_summary=ws_model,   # ✅ stays WeatherSummary
                max_activities=max_activities,
                arrival_time=arrival_time if is_first and flight else None,
                departure_time=departure_time if is_last and flight else None,
                available_hours=available_hours,
                intensity_target=intensity,
            )
        )
...
def allocate_time_blocks(places: List[PlaceOption], 
    ...
    for place in places:
        ...
        place_obj = PlaceOption.model_validate(
...
def build_day_schedule(
    ...
    # Normalize scheduled PlaceOption objects to the PlaceOption type used by THIS module
    scheduled_fixed: list[PlaceOption] = []
    for p in scheduled:
        if hasattr(p, "model_dump"):
            scheduled_fixed.append(PlaceOption.model_validate(p.model_dump()))
        else:
            scheduled_fixed.append(PlaceOption.model_validate(p))
    ...
    return DaySchedule(
        ...
        activities=[
            PlaceOption.model_validate(p.model_dump() if hasattr(p, "model_dump") else p)
            for p in scheduled
        ],
...
def enrich_itinerary(
    ...
    metrics = ItineraryPlanningMetrics(
        ...
        budget_remaining_after_activities=int(round(float(budget_total or 0) - float(total_activity_cost or 0))),
